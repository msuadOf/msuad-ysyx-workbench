...
#define ECALL(dnpc)                                                                            \
  {                                                                                            \
    bool success;                                                                              \
    dnpc = (isa_raise_intr(isa_reg_str2val(MUXDEF(CONFIG_RVE, "a5", "a7"), &success), s->pc)); \
    cpu.csr.mcause = 0xb;                                                                      \
  }
...
{
  assert(ext >= 'A' && ext <= 'Z');
  return (MISA_INITVAL() >> (ext - 'A')) & 1;
}

static inline word_t legalize_privilege(word_t prv)
{
  assert(prv <= PRV_M);
...
static void _mret(Decode *d)
{
  d->dnpc = cpu.csr.mepc;

  word_t s = cpu.csr.mstatus;
  word_t prev_prv = get_field(s, MSTATUS_MPP);
  word_t prev_virt = get_field(s, MSTATUS_MPV);
  if (prev_prv != PRV_M)
    s = set_field(s, MSTATUS_MPRV, 0);
  s = set_field(s, MSTATUS_MIE, get_field(s, MSTATUS_MPIE));
  s = set_field(s, MSTATUS_MPIE, 1);
  s = set_field(s, MSTATUS_MPP, extension_enabled('U') ? PRV_U : PRV_M);
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  // uncompletely  tested
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I, R(rd) = SEXT(Mr(src1 + imm, 4), 8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh, I, R(rd) = SEXT(Mr(src1 + imm, 4), 16));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw, I, R(rd) = Mr(src1 + imm, 4));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu, I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu, I, R(rd) = Mr(src1 + imm, 2));

  //
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi, I, R(rd) = src1 + imm); // 重复了
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra, R, R(rd) = (sword_t)SEXT(src1, 32) >> BITS(src2, 4, 0));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R, R(rd) = ((uint64_t)(((int64_t)(int32_t)src1) * ((int64_t)(int32_t)src2))) >> 32);
