...
word_t mmio_read(paddr_t addr, int len) {
    ...
    doREAD(SERIAL);
    doREAD(RTC);
    IFNDEF(CONFIG_MMIO_NO_CHECK,panic("[npc]mmio_read should not reach here!");)
    return -1;
}
...
void mmio_write(paddr_t addr, int len, word_t data) {
    ...
    doWRITE(SERIAL);
    doWRITE(RTC);
    IFNDEF(CONFIG_MMIO_NO_CHECK,panic("[npc]mmio_write should not reach here! addr=%08x,len=%d,data=%08x",addr,len,data);)
}
